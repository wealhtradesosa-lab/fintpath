...
// 04-sep-2026 (Santiago: "pongamos buenos disclaimers, no soy responsable de
// una mala decisión de la gente, esto es un insumo").
// Los Excel salían sin ningún aviso. Es donde más falta: un archivo se reenvía
// por correo, se abre meses después y se lee fuera de la plataforma, sin nada
// del contexto que en pantalla sí está.
// Va como PRIMERA hoja del libro para que sea lo primero que se ve al abrirlo.
const agregarHojaAviso = (XLSX, wb, X) => {
  const ws = XLSX.utils.aoa_to_sheet(X.aviso);
  ws["!cols"] = [{ wch: 115 }];
  XLSX.utils.book_append_sheet(wb, ws, X.shAviso);
};
...
const XL = {
  es: {
    ...
    shAviso: "Aviso legal",
    aviso: [
      ["AVISO LEGAL — LEER ANTES DE USAR ESTE ARCHIVO"],
      [""],
      ["Este archivo lo generó FINPATHIA a partir de los datos que el usuario cargó y de los supuestos que eligió."],
      [""],
      ...
      ["5. LA DECISIÓN ES TUYA. Cada persona es responsable de las decisiones que tome y de sus resultados."],
      ["   FINPATHIA es un insumo para pensar, no un consejo a seguir."],
      [""],
      ["Este archivo puede contener información financiera personal. Compartilo con cuidado."],
    ],
  ...
  en: {
    ...
    shAviso: "Disclaimer",
    aviso: [
      ["DISCLAIMER — READ BEFORE USING THIS FILE"],
      [""],
      ["This file was generated by FINPATHIA from the data the user entered and the assumptions they chose."],
      [""],
      ["1. NOT ADVICE. These figures do not constitute financial, tax, accounting or legal advice, nor a"],
      ["   recommendation to buy or sell any asset."],
      ["2. NOT A SUBSTITUTE FOR A PROFESSIONAL. Before making any decision or filing anything with a tax"],
      ["   authority, verify every figure with a licensed professional who knows your full situation."],
      ["3. THE DATA IS THE USER'S. FINPATHIA does not audit or verify what is entered. If an input is wrong,"],
      ["   the output will be wrong too."],
      ["4. PROJECTIONS ARE NOT FORECASTS. Past performance does not guarantee future results."],
      ["5. THE DECISION IS YOURS. Each person is responsible for their own decisions and their outcomes."],
      ["   FINPATHIA is an input for thinking, not advice to follow."],
      [""],
      ["This file may contain personal financial information. Share it carefully."],
    ],
