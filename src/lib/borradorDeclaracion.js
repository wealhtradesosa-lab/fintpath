...
export function generarBorradorF110(user, owner, estimacion, ano = 2025) {
  ...
  const renglones = [
    ...
    { seccion: "patrimonio", numero: 36, concepto: "Efectivo y equivalentes al efectivo", valor: v(36, efectivo), auto: efectivo, tipo: "editable", fuente: "Inversiones tipo cuenta bancaria", articulo: "Art. 261 ET",
      tip: "🏦 Saldo de las cuentas bancarias de la sociedad (corrientes y de ahorros) al 31 de diciembre. Aquí NO van CDT ni inversiones (esos van al renglón 37). Si tienes cuentas en USD, convierte a TRM del 31-dic." },
    ...
    { seccion: "patrimonio", numero: 38, concepto: "Cuentas, documentos y arrendamientos por cobrar", valor: v(38, 0), auto: 0, tipo: "editable", fuente: "Manual (no cargado)", articulo: "Art. 261 ET",
      tip: "📋 Cuentas por cobrar a clientes, préstamos a terceros, anticipos a proveedores, arriendos pendientes de cobro al 31-dic. ⚠️ Si tienes provisión de cartera, va el saldo NETO (cuentas - provisión) y la provisión va como deducción." },
    ...
    { seccion: "impuesto", numero: 84, concepto: "Impuesto sobre las rentas líquidas gravables (35%)", tipo: "formula", destacado: true,
      ...
      tip: "🏛️ Tarifa general del impuesto a la renta para sociedades: 35%. ⚠️ Si tu sociedad está en Régimen SIMPLE de Tributación, NO aplica el 35% sino una tarifa entre 1.2% y 14.5% según actividad económica. Para zonas francas: 20%, ZESE: 0% primeros 5 años. Si cambiaste de régimen, ajusta manualmente.",
    ...
    { seccion: "liquidacion", numero: 105, concepto: "Autorretenciones", valor: v(105, 0), auto: 0, tipo: "editable", fuente: "Manual (certificados autorretención)", articulo: "Art. 365 ET",
      tip: "📋 Si la sociedad es AUTORRETENEDORA (resolución DIAN), debe practicarse retención a sí misma sobre sus ingresos. Sumar todas las autorretenciones declaradas y pagadas durante el año vía formulario 350. ⚠️ Solo si eres autorretenedor, sino va $0." },
    ...
    { seccion: "liquidacion", numero: 111, concepto: "Saldo a pagar por impuesto", tipo: "formula", destacado: true,
      ...
      tip: "💸 Saldo final a pagar. Si es positivo: pagas esa cantidad. Si es 0 o negativo: aplica saldo a favor (renglón 114). El anticipo del año siguiente (108) SE SUMA porque también hay que pagarlo con esta declaración.",
    ...
    { seccion: "liquidacion", numero: 113, concepto: "TOTAL SALDO A PAGAR", tipo: "formula", destacado: true,
      formula: "111 + 112",
      tip: "💰 Total a pagar a la DIAN: saldo del impuesto + sanciones (si aplica). Este es el monto que debes pagar antes del vencimiento. Recuerda que el pago se puede fraccionar en cuotas si corresponde.",
    ...
    { seccion: "liquidacion", numero: 114, concepto: "TOTAL SALDO A FAVOR", tipo: "formula",
      ...
      tip: "💚 Si el cálculo resulta negativo en el renglón 111 (pagaste más en retenciones/anticipos que el impuesto), aparece aquí el saldo a favor. Puedes solicitar devolución a la DIAN o imputarlo al año siguiente (renglón 104 del próximo año).",
