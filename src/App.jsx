...
export default function FinPath(){
  ...
  const demo=()=>{showToast("📊 Datos de ejemplo cargados");window.gtag?.("event","demo_opened",{method:"button"});setU(p=>{
    ...
    nd.p={name:"Pedro Pérez",email:"demo@finpathia.com",plan:"pro",trialEnd:new Date(Date.now()+30*86400000).toISOString().split("T")[0],demo:true,anonymous:true};
    nd.owners=[
  ...
  const nvs=[{id:"dash",i:"📊",l:"Dashboard"},{id:"_sep1",sep:true,l:isUS?"MY MONEY":"MI DINERO"},{id:"ing",i:"💰",l:isUS?"Income":"Ingresos"},{id:"gas",i:"💳",l:isUS?"Deductions":"Egresos"},{id:"inv",i:"🏦",l:isUS?"Assets":"Patrimonio"},{id:"deu",i:"📋",l:isUS?"Liabilities":"Deudas"},{id:"tax",i:"🧾",l:isUS?"Tax Planning":"Plan Tributario"},{id:"_sep2",sep:true,l:isUS?"TOOLS":"HERRAMIENTAS"},{id:"sim",i:"🖥️",l:isUS?"Simulator":"Simulador"},{id:"met",i:"🎯",l:isUS?"Goals":"Metas"},{id:"trd",i:"💹",l:"Trading"},{id:"pen",i:"🏛️",l:isUS?"Retirement / 401(k)":"Pensiones"},{id:"btc",i:"₿",l:"Ahorro BTC"},{id:"aportes",i:"💰",l:"Calcula tus aportes"},{id:"_sep3",sep:true,l:isUS?"ARTIFICIAL INTELLIGENCE":"INTELIGENCIA ARTIFICIAL"},{id:"asesor",i:"🤖",l:isUS?"AI Advisor":"Asesor IA"},{id:"coach",i:"🧠",l:isUS?"AI Coaches":"Coaches IA"},{id:"_sep4",sep:true},{id:"price",i:"⭐",l:"Planes"},{id:"set",i:"⚙️",l:"Config"}];
...
case"inv":return isUS?<AssetsModuleUS inversiones={(u&&u.inv)||[]} deudas={(u&&u.deu)||[]} onUpdateAssets={v=>upd("inv",v)} onUpdateLiabs={v=>upd("deu",v)}/>:<InversionesModule owners={u?.owners||[]} inversiones={(u&&u.inv)||[]} deudas={(u&&u.deu)||[]} onUpdate={v=>upd("inv",v)} fmt={fm} onImport={()=>setShowImport(true)}/>;
    ...
    case"trd":return gated("trd","Básico",<div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><h2 style={{fontSize:22,fontWeight:700,margin:0}}>Trading</h2><Bt sz="s" onClick={async()=>{
        ...
        case"gas":return isUS?<ExpensesModuleUS gastos={(u&&u.gas)||{}} onUpdate={v=>upd("gas",v)} agi={totals.ti*12}/>:<GastosModule owners={u?.owners||[]} gastos={(u&&u.gas)||{}} onUpdate={v=>upd("gas",v)} fmt={fm} onImport={()=>setShowImport(true)}/>;
        case"deu":return isUS?<AssetsModuleUS inversiones={(u&&u.inv)||[]} deudas={(u&&u.deu)||[]} onUpdateAssets={v=>upd("inv",v)} onUpdateLiabs={v=>upd("deu",v)}/>:<DeudasModule owners={u?.owners||[]} deudas={(u&&u.deu)||[]} inversiones={(u&&u.inv)||[]} onUpdate={v=>upd("deu",v)} fmt={fm} onImport={()=>setShowImport(true)}/>;
    case"met":return<MetasModule metas={(u&&u.metas)||[]} onUpdate={v=>upd("metas",v)} cashFlow={t.cf} fmt={fm}/>;
    case"sim":return<SimuladorAvanzado impuestoData={estimarImpuesto(u)} user={{inv:(u&&u.inv)||[],gastos:(u&&u.gas)||{},deudas:(u&&u.deu)||[],ibkr:(u&&u.ibk)||[],ingresos:(u&&u.ingresos)||[],owners:(u&&u.owners)||[{id:"own_1",name:"Personal",type:"natural"}]}} totals={t} fmt={fm}/>;
    ...
    case"pen":return isUS?<div style={{padding:48,textAlign:"center",color:T.tx3}}><div style={{fontSize:32,marginBottom:12}}>🏛️</div><div style={{fontSize:16,fontWeight:700,color:T.tx2,marginBottom:8}}>Retirement Planning</div><div style={{fontSize:13,marginBottom:20}}>401(k), IRA, Roth IRA, Social Security tracker coming soon.</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,maxWidth:500,margin:"0 auto"}}>{[{l:"401(k) Limit",v:"$23,500",sub:"2025"},{l:"IRA Limit",v:"$7,000",sub:"Traditional + Roth"},{l:"SS Full Age",v:"67",sub:"Born after 1960"}].map(k=><div key={k.l} style={{background:T.card,border:"1px solid "+T.border,borderRadius:12,padding:16}}><div style={{fontSize:10,color:T.tx3}}>{k.l}</div><div style={{fontSize:20,fontWeight:700,color:T.gn,marginTop:4}}>{k.v}</div><div style={{fontSize:10,color:T.tx3}}>{k.sub}</div></div>)}</div></div>:gated("pen","Básico",<PensionesColpensiones trm={(u&&u.trm)||4200}/>);
