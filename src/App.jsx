...
export default function FinPath(){
  ...
  if(!u)return<div style={{background:T.bg,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter',system-ui",color:T.tx}}>
    ...
    <div style={{width:"100%",maxWidth:420,padding:"clamp(24px, 6vw, 40px) clamp(20px, 5vw, 32px)"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24,gap:12,flexWrap:"wrap"}}><div onClick={()=>setShowAuth(false)} style={{fontSize:13,color:T.tx3,cursor:"pointer"}}>← Volver</div>{aM==="signup"&&<button type="button" onClick={()=>sAM("login")} style={{background:"transparent",border:`1px solid ${T.gn}55`,color:T.gn,padding:"6px 12px",borderRadius:8,cursor:"pointer",fontWeight:700,fontSize:13}}>Ya tengo cuenta · Iniciar sesión</button>}</div>
