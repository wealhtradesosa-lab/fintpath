...
export default function FinPath(){
  ...
  const getCoach=id=>{
    ...
    if(id==="cashflow"){
      ...
      const inferType=(i)=>{let tp=i.tp||i.tipo||"";if(!tp||!isNaN(Number(tp)))tp="";if(tp)return tp;const nm=((i.n||i.nombre||"")+" "+(i.ub||"")).toLowerCase();if(/apart|apto|casa|lote|terreno|oficina|inmueble|propiedad/i.test(nm))return"Real Estate";if(/bodega/i.test(nm))return"Bodega";if(/local/i.test(nm))return"Local Comercial";if(/fondo|fiduci/i.test(nm))return"Fondo de Inversión";if(/cdt|renta fija|bonos|tes /i.test(nm))return"CDT";if(/accion|etf|portafolio|vti|spy|stock/i.test(nm))return"Acciones";if(/btc|bitcoin|crypto|eth/i.test(nm))return"Crypto";if(/vehic|carro|moto/i.test(nm))return"Vehículo";if(/negocio|empresa|sas/i.test(nm))return"Negocio";if(/cash|ahorro|cuenta/i.test(nm))return"Cash";return"Otro"};
