...
import AdvisorModeSelector from "./components/AdvisorModeSelector";
import AcceptInvite from "./components/AcceptInvite";
import IngresosModule from "./components/IngresosModule";

// Build tag: sprint-2b-selector-2026-04-22
const __FINPATHIA_BUILD_ID__ = "fp-build-sprint-2b-selector-20260422";
...
export default function FinPath(){
  ...
  const[viewMode,setViewMode]=useState("selector"); // "selector" | "workspace" | "client" | "personal"
  ...
  const logout=async()=>{try{await supabase.auth.signOut()}catch{}localStorage.removeItem(SK);localStorage.removeItem("fp3_enc_key");_setU(null);setShowAuth(false);setIsAdvisor(false);setAdvisorProfile(null);setAdvisorClients([]);setViewMode("selector");setCurrentClientId(null)};
  const auth=async()=>{
  ...
  // Si el usuario es advisor y acaba de loguearse (viewMode="selector"),
  // le mostramos una pantalla para elegir entre modo asesor y modo personal.
  // Se muestra CADA vez que se loguea (no persiste la elección).
  if(u&&isAdvisor&&viewMode==="selector"){
    return<AdvisorModeSelector
      advisorProfile={advisorProfile}
      clientCount={advisorClients.length}
      userName={u?.p?.name||authUser?.email}
      onSelectWorkspace={()=>setViewMode("workspace")}
      onSelectPersonal={()=>setViewMode("personal")}
      onLogout={logout}
    />;
  }
...
case"inv":return isUS?<AssetsModuleUS inversiones={(u&&u.inv)||[]} deudas={(u&&u.deu)||[]} onUpdateAssets={v=>upd("inv",v)} onUpdateLiabs={v=>upd("deu",v)} initialTab="assets"/>:<InversionesModule owners={u?.owners||[]} inversiones={(u&&u.inv)||[]} deudas={(u&&u.deu)||[]} onUpdate={v=>upd("inv",v)} fmt={fm} onImport={()=>setShowImport(true)}/>;
