...
const cT=(inv,ds,gf,ing)=>{let ab=0,ti=0,tg=0;(inv||[]).forEach(i=>{ab+=i.va});const ingT=(ing||[]).reduce((s,i)=>s+(i.mensual||0),0);ti=ingT;const td=(ds||[]).reduce((s,d)=>s+(d.mt||0),0),tc=(ds||[]).filter(d=>(d.mt||0)>0).reduce((s,d)=>s+(d.pg||0),0),gfm=Object.values(gf||{}).flat().reduce((s,g)=>s+(g.m||0),0),ni=ti-tg,te=gfm+tc,cf=ni-te;return{ab,td,nw:ab-td,ti,tg,ni,gfm,tc,te,cf,ind:te>0?(ni/te)*100:0,dta:ab>0?(td/ab)*100:0,ingT}};
...
export default function FinPath(){
  ...
  const gateOverlay=(planNeeded)=><div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",zIndex:10}}><div style={{background:T.bg2,border:"1px solid "+T.border,borderRadius:16,padding:"32px 40px",textAlign:"center",boxShadow:"0 8px 32px rgba(0,0,0,.4)"}}><div style={{fontSize:32,marginBottom:8}}>🔒</div><div style={{fontSize:16,fontWeight:700,marginBottom:4}}>{"Feature "+planNeeded}</div><div style={{fontSize:13,color:T.tx3,marginBottom:16}}>{"Disponible en plan "+planNeeded}</div><Bt onClick={()=>setPg("price")}>Ver Planes</Bt></div></div>;
  ...
  if(!u)return<div style={{background:T.bg,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter',system-ui",color:T.tx}}>
    ...
    <div style={{width:"100%",maxWidth:420,padding:"40px 32px"}}>
      ...
      <div style={{background:"rgba(34,197,94,0.06)",border:"1px solid rgba(34,197,94,0.15)",borderRadius:10,padding:"10px 14px",marginBottom:20,fontSize:12,color:T.tx2,display:"flex",alignItems:"center",gap:8}}>🔒 Tus datos están protegidos con encriptación.</div>
