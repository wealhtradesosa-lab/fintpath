...
export default function SimuladorAvanzado({ user, totals }) {
  ...
  const applyScenario = (id) => {
    ...
    (user.inv || []).forEach((inv) => {
      const items = inv.unidades
        ? inv.unidades.flatMap((u, ui) => [
            ...(u.ingresos || []).map((ig, ii) => ({ key: `i_${inv.id}_u${ui}_${ii}`, base: ig.m, isI: true })),
            ...(u.gastos || []).map((g, gi) => ({ key: `g_${inv.id}_u${ui}_${gi}`, base: g.m, isI: false })),
          ])
        : [
            ...(inv.ingresos || []).map((ig, ii) => ({ key: `i_${inv.id}_${ii}`, base: ig.m, isI: true })),
            ...(inv.gastos || []).map((g, gi) => ({ key: `g_${inv.id}_${gi}`, base: g.m, isI: false })),
          ];
    ...
    (user.deudas || []).forEach((d, di) => { nv[`debt_${di}`] = d.pago; });
    setSimVals(nv);
  };
  ...
  const simT = useMemo(() => {
    ...
    (user.inv || []).forEach((inv) => {
      if (inv.unidades) {
        inv.unidades.forEach((u, ui) => {
          (u.ingresos || []).forEach((ig, ii) => { tI += getVal(`i_${inv.id}_u${ui}_${ii}`, ig.m); });
          (u.gastos || []).forEach((g, gi) => { tG += getVal(`g_${inv.id}_u${ui}_${gi}`, g.m); });
        });
      } else {
        (inv.ingresos || []).forEach((ig, ii) => { tI += getVal(`i_${inv.id}_${ii}`, ig.m); });
        (inv.gastos || []).forEach((g, gi) => { tG += getVal(`g_${inv.id}_${gi}`, g.m); });
      }
    });
    let tGF = 0;
    Object.entries(user.gastos || {}).forEach(([cat, items]) => {
    ...
    let tD = 0;
    (user.deudas || []).forEach((d, di) => { tD += getVal(`debt_${di}`, d.pago); });
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div style={{ maxHeight: "70vh", overflowY: "auto", paddingRight: 8 }}>
          ...
          {(user.inv || []).map((inv) => {
            const items = inv.unidades
              ? inv.unidades.flatMap((u, ui) => [
                  ...(u.ingresos || []).map((ig, ii) => ({ key: `i_${inv.id}_u${ui}_${ii}`, label: u.nombre + ": " + ig.c, base: ig.m, tp: "i" })),
                  ...(u.gastos || []).map((g, gi) => ({ key: `g_${inv.id}_u${ui}_${gi}`, label: u.nombre + ": " + g.c, base: g.m, tp: "g" })),
                ])
              : [
                  ...(inv.ingresos || []).map((ig, ii) => ({ key: `i_${inv.id}_${ii}`, label: ig.c, base: ig.m, tp: "i" })),
                  ...(inv.gastos || []).map((g, gi) => ({ key: `g_${inv.id}_${gi}`, label: g.c, base: g.m, tp: "g" })),
                ];
            ...
            return (
              <div key={inv.id} style={{ marginBottom: 10, background: "rgba(255,255,255,0.02)", borderRadius: 10, border: "1px solid " + T.gn + "20", overflow: "hidden" }}>
                <div style={{ padding: "8px 12px", background: T.bg2, borderBottom: "1px solid " + T.gn + "15", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <span style={{ fontSize: 13, fontWeight: 700 }}>{inv.nombre}</span>
                    <span style={{ fontSize: 10, color: T.txt3, marginLeft: 6 }}>{inv.ubi || inv.ubicacion}</span>
                  </div>
          ...
          {(user.deudas || []).map((d, di) => {
            ...
            return (
              <Slider key={`debt_${di}`} label={d.nombre || d.n} value={getVal(`debt_${di}`, d.pago || d.pg)} base={d.pago || d.pg}
                max={Math.max((d.pago || d.pg) * 3, 500)} color={T.pr}
                onChange={(v) => setVal(`debt_${di}`, v)}
                sub={lk ? "→ " + (lk.nombre || lk.n) : (d.tasa || d.ts) > 0 ? (d.tasa || d.ts) + "%" : ""} />
            );
          })}
